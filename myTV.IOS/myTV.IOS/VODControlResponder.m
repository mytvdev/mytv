...
-(void)bindData:(NSObject *)data {
    self.program = (MyTVProgram *)data;
    VODControlResponder *parent = self;
    self.imageFetcher = [DataFetcher Get:self.program.Logo usingCallback:^(NSData *data, NSError *error){
        if(error == nil && data != nil) {
            parent.imageDisplay.image = [[UIImage alloc] initWithData:data];
        }
    }];
    labelDisplay.text = self.program.Title;
...
-(void) fireEvent:(UIGestureRecognizer *)gestureRecognizer {
    UIAlertView *view = [[UIAlertView alloc] initWithTitle:@"Play Channel" message:[NSString stringWithFormat:@"Do you want to view %@?", self.program.Title] delegate:self cancelButtonTitle:@"No" otherButtonTitles:@"Yes", nil];
    [view show];
}
